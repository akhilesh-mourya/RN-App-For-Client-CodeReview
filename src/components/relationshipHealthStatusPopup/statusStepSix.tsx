...
export const StatusStepSix: FC<ScreenProps> = React.memo(props => {
  const {
    step,
    updateStep,
    progressList,
  ...
  const renderImage = () => {
    return (
      <StylesImgContainer>
        <StatusImg source={HealthStatus.RELATIONSHIP_SUMMARY} />
      </StylesImgContainer>
    );
  };
  ...
  const renderStyleDetail = () => {
    return (
      <StylesDetailContainer>
        <SummaryStyleLabel>{t('Summary_Title')}</SummaryStyleLabel>
        <ScrollingText>
