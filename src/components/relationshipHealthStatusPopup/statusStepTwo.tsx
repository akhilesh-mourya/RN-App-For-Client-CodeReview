...
export const StatusStepTwo: FC<ScreenProps> = React.memo(props => {
  const {
    step,
    updateStep,
    progressList,
  ...
  const renderImage = () => {
    return (
      <StylesImgContainer>
        <StatusImg source={HealthStatus.RELATIONSHIP_STRENGTH} />
      </StylesImgContainer>
    );
  };
  ...
  const renderStyleDetail = () => {
    return (
      <StylesDetailContainer>
        <SummaryStyleLabel>{t('Strength_In_Relationship')}</SummaryStyleLabel>
        <ScrollingText>
